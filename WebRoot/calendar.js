...
function getParentOffset(el, positions)
{
	...
	function position(dateDMY)
	{
		if ((dateDMY == null) ) return "";
		var len=dateDMY.length;
		if (!(len == 10))  return "";
		var pos1 = dateDMY.substring(2,3); 
		var date=pos1;
		return date;
	}
	function position1(dateDMY1)
	{
		if ((dateDMY1 == null) ) return "";
		var len=dateDMY1.length;
		if (!(len == 10))  return "";
		var pos2 = dateDMY1.substring(5,6); 
		var date1=pos2;
		return date1;
	}
	function convertToYMD(dateDMY)
	{
		if ((dateDMY == null) ) return "";
		var len=dateDMY.length;
		if (!(len == 10))  return "";
		var dd = dateDMY.substring(0, 2); 
		var mm = dateDMY.substring(3,5);
		var yyyy = dateDMY.substring(6,10);
		var ymd = yyyy+mm+dd;
		return ymd;
	}
//Comparasion of two dates of Date dd-mm-yyyy 
	function Comparedates(dateDMYfirst,dateDMYsecond)
	{
		var ymd1=convertToYMD(dateDMYfirst);
		var ymd2=convertToYMD(dateDMYsecond);
